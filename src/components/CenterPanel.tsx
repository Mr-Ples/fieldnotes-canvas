...
export default function CenterPanel() {
  ...
  const visibleAnnotationMode = tab === 'resources' ? 'hidden' : annotationMode
  ...
  return <main ref={centerRef} className="center-panel" id="top">
    ...
    <div hidden={tab !== 'notes'}>
      <Notes key={activeCanvas.id} canvasId={activeCanvas.id} setSaved={setSaved} containerRef={centerRef} canInteract={canUseCanvas} canSaveResource={memberAccess.resources} annotationMode={visibleAnnotationMode} onPendingHeadingScroll={(target) => { pendingHeadingScroll.current = target }} />
    </div>
...
function Comments({ canInteract, canSaveResource }: { canInteract: boolean; canSaveResource: boolean }) {
  ...
  return <section className="comments-section"><div className="section-title"><h2>Discussion <span>{items.length}</span></h2></div>
    <div className="comment-list">{items.map((comment) => <CommentItem key={comment.id} comment={comment} replyTargetId={replyTarget?.id} currentUserId={currentAuthorId} currentName={currentAuthor} canInteract={canInteract} canSaveResource={canSaveResource} onReply={reply} onSave={saveAsResource} onCopy={(id) => void copyLink(id)} onDelete={remove}/>)}</div>
...
function CommentItem({ comment, replyTargetId, currentUserId, currentName, canInteract, canSaveResource, onReply, onSave, onCopy, onDelete }: { comment: Comment; replyTargetId?: string; currentUserId: string; currentName: string; canInteract: boolean; canSaveResource: boolean; onReply: (comment: Comment) => void; onSave: (comment: Comment) => void; onCopy: (id: string) => void; onDelete: (id: string) => void }) {
